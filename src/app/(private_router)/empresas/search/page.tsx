...
export default function EmpresaSearch() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const urlVendedor = `/empresas?filters[user][username][$containsi]=${session?.user.name}&filters[nome][$containsi]=${urlSearch.get('titulo')}&sort[0]=nome%3Aasc&fields[0]=nome&populate[user][fields][0]=username&populate[businesses]=*&populate[interacaos][fields][0]=proxima&populate[interacaos][fields][1]=vendedor_name&populate[interacaos][fields][2]=status_atendimento`;
