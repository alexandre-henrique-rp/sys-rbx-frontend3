...
export const getData = async (id: any) => {
  ...
  // const url = `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/pedidos?populate=*&filters[nPedido][$eq]=${id}`;
  const url = `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/pedidos/${id}?populate=*`;
  ...
  try {
    ...
    const dadosFornecedor = {
      data: {
        ...
        cnpj: empresaFornec?.CNPJ.replace(/^(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})$/, "$1.$2.$3/$4-$5"),
    ...
    const Desconto_Converte = !inf.desconto
      ? null
      : inf.desconto;
    const Desconto = !Desconto_Converte
      ? 0
      : Desconto_Converte;
    const DescontoAdd_Converte = !inf.descontoAdd
      ? null
      : parseFloat(inf.descontoAdd)
    const DescontoAdd = !DescontoAdd_Converte
      ? 0
      : DescontoAdd_Converte
    const itens = inf.itens;
    console.log("🚀 ~ file: getinf.ts:70 ~ getData ~ inf.prazo:", inf.prazo)
    const prazo = inf.prazo === null
      ? ""
      : inf.prazo;
    const venc = inf.vencPrint;
    const totoalGeral = inf.totalGeral;
    const obs = inf.obs === null
      ? ""
      : inf.obs;
