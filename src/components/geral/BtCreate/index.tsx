...
export const BtCreate = (props: { user: any; onLoading: any }) => {
  ...
  useEffect(() => {
    ...
    if (usuario) {
      (async () => {
        try {
          const response = await FetchRequest.get(`/empresas?filters[user][username][$eq]=${usuario}&filters[status][$eq]=true&sort[0]=nome%3Aasc&fields[0]=nome&fields[1]=CNPJ&populate[user][fields][0]=username&populate[businesses][fields][0]=nBusiness`);
          const response1 = await FetchRequest.get(`/empresas?filters[user][username][$null]=true&filters[status][$eq]=true&sort[0]=nome%3Aasc&fields[0]=nome&fields[1]=CNPJ&pagination[start]=0&pagination[limit]=5000`);
