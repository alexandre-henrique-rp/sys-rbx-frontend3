...
import { FormaPg } from "@/components/geral/formaDePagameto/page";
import { confgEnb } from "@/data/configEmbalagem";
import { modCaix } from "@/data/modeloCaixa";
import { CompPessoa } from "@/components/geral/compPessoa/page";
...
export const FormEmpresa = (props: { data?: any, envio: string, id?: any }) => {
  ...
  const consulta = async () => {
    if (CNPJ) {
      ...
      const teset = await FetchRequest.get(`/empresas/?filters[CNPJ][$eq]=${CNPJ}&populate=*`)
  ...
  const save = async () => {
    if (nome === "") {
      ...
      if (ENVIO === 'POST') {
        try {
          const response = await PostEmpresa(dataUpdate)
          if (response) {
            toast({
        ...
        try {
          const EMPRESAID: any = ID;
          const response = await PutEmpresa(dataUpdate, EMPRESAID)
          if (response.data) {
            toast({
                        ...
                        <FormControl as={GridItem} colSpan={[6, 4, null, 3]}>
                          ...
                          <Input
                            ...
                            onChange={(e) => {
                              const valor = e.target.value;
                              setCelular(valor);
                              setWhatsMask(formatarTelefone(valor));
                            }}
                            value={WhatsMask}
                          />
                        </FormControl>
