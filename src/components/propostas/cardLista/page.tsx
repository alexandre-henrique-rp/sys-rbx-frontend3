...
export const GetPropostas = async (id: any) => {
  const requeste = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/db/proposta/get/business/${id}`, {
    headers: {
...
export const CardList = (props: { id: string; onloading: any; desbilitar: any }) => {
  ...
  useEffect(() => {
    (async () => {
      setLoadGeral(true)
      const requeste = await GetPropostas(props.id);
